...
<head>
    <meta charset="UTF-8">
    <link rel="icon" href="icons-96.png" type="image/png">
...
?>
        ...
        <div class="left-block">
            ...
            <div style="overflow: auto">
                ...
                foreach ($allUsers as $userNickname) {
                    ...
                    ?>
                    <div class="leftmess" onclick="storeUserInfo('<?php echo $userNickname ?>', '<?php echo $userImage ?>')">
                        <a href="">
                            <div class="circle">
                                <img src="../images/users/<?php echo $userImage ?>" alt="profile image">
                            </div>
                        </a>
                        <div class="mess-text">
                            <div class="nick-text"><?php echo $userNickname ?></div>
                            <div class="mess">Active now</div>
                        </div>
                    </div>
                <?php } ?>
                ...
                <div class="top-left">
                    <a href="">
                        <div class="prof-circle">
                            <img id="profileImage" src="../images/users/<?php echo $userImage?>" alt="profile image">
                        </div>
                    </a>
                    <div class="nick-top">Name Surname</div>
                </div>
                <div class="top-right">
    ...
    <script>
        ...
        "У вас есть какие-то планы на сегодня?"
    ];
    function sendMessage() {
        ...
        if (message !== "") {
            ...
            messageInput.value = "";
            // toggleSendButton();
            // setTimeout(sendBotResponse, 3000); // Отправляем ответ бота с задержкой
        }
    ...
    function appendMessage(message, isUser) {
        var messageContainer = document.getElementById("mid-block");
        var messageElement = document.createElement("div");
        messageElement.className = isUser ? "user-message" : "message";

        if (!isUser) { // Если сообщение от бота, добавляем фото
            var botImage = document.createElement("p");
    ...
    messageContainer.innerHTML = "";
        ...
        function storeUserInfo(nickname, image) {
            // Use localStorage to store user information
            localStorage.setItem('selectedUserNickname', nickname);
            localStorage.setItem('selectedUserImage', image);
            clearMessages();

            // You can also redirect or perform any other actions here
        }

        function updateNicknameDiv() {
            var storedNickname = localStorage.getItem('selectedUserNickname');

            // Update the 'Name Surname' div with the selected user's nickname
            var nickTopDiv = document.querySelector('.nick-top');
            if (nickTopDiv) {
                nickTopDiv.textContent = storedNickname;
            }
        }

        function updateProfileImage() {
            // Retrieve stored information
            var storedImage = localStorage.getItem('selectedUserImage');

            // Update the profile image source dynamically
            var profileImage = document.getElementById('profileImage');
            if (profileImage) {
                profileImage.src = '../images/users/' + storedImage;
            }
        }

        // Initial calls to updateNicknameDiv and updateProfileImage to set the initial values
        updateNicknameDiv();
        updateProfileImage();
</script>
